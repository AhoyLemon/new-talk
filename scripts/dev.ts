...
function getProjectName(): string {
  try {
    ...
    return packageJson.name || "Your Project";
  } catch {
    return "Your Project";
  }
}
...
function checkAllReady(spinner: any) {
  if (browserSyncReady && sassReady && pugReady && !hasErrors) {
    // Give it a moment for BrowserSync to fully start
...
function startProcess(
  ...
  proc.stdout.on("data", (data: Buffer) => {
    ...
    if (browserSyncReady && sassReady && pugReady) {
      ...
      if (shouldShow) {
        // Don't clear, just append with timestamp-aware formatting
  ...
  proc.stderr.on("data", (data: Buffer) => {
    ...
    // Some processes write normal info to stderr, check before treating as error
    // Sass often writes its "watching" messages to stderr
    if (
      name === "Sass" &&
      !sassReady &&
      (output.includes("Sass is watching") ||
    ...
    // Only treat as error if it actually contains error keywords
    if (
  ...
  // After a short delay, assume the process is ready if it hasn't crashed
  setTimeout(() => {
    if (!hasOutput && proc.exitCode === null) {
      // Process is running but silent, assume it's ready
...
async function startDev() {
  ...
  startProcess(
    ...
    [
      ...
      "--server",
      "--files",
      "*.html, css/*.css, js/**/*.js, demos/**/*.html, demos/css/*.css, demos/js/*.js",
      "--no-notify",
      "--no-open",
    ],
    spinner
  ...
  // Timeout check - if not ready in 10 seconds, something is wrong
  setTimeout(() => {
    if (!browserSyncReady || !sassReady || !pugReady) {
      spinner.warn(
